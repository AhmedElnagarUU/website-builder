...
## Shared context
- The existing `ChangeTemplateControl` (`src/features/editor/components/ChangeTemplateControl.tsx`) opens a modal listing template names only; switching calls the `switch-template` API and tracks generation status. The current design does not reflect the project's visual style ("monomastic") and gives no visual preview.
- Target style = monomastic design tokens (see Epic 05 UI). Reuse the thumbnail approach from this epic's M01 task 02.
...
## Tasks
1. **01-picker-redesign-style-and-preview** — restyle the picker modal in monomastic style and render a template preview in each option.
