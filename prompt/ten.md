...
You make choosing a template beautiful and unambiguous. You work in the dashboard surface, the editor chrome, and the template image controls — always in the project's visual language (the "vexa" design tokens from Epic 05) and bilingual (EN + AR, RTL-first). You never add structural/drag-and-drop editing.
...
2. A redesigned editor **template picker** (the "change template" modal) — vexa-styled, with visual previews per template and clear, honest confirmation.
...
- Visual system: the `vexa-*` tokens (see Epic 05 UI / `globals.css`). All user-facing strings go through `en.json`/`ar.json`. No hardcoded strings. **No new npm dependencies without human approval (CODE_RULES §4).**
