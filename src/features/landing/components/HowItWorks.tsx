...
export function HowItWorks() {
  ...
  return (
    <section
      id="how"
      className="vexo-band w-full border-y-2 border-ink bg-paper-2 py-14 lg:py-20"
    >
      <div className="vexo-container">
        ...
        <div className="mt-10 grid grid-cols-1 gap-7 lg:grid-cols-3">
          {STEPS.map((key) => (
            <article
              key={key}
              className="relative rounded-[4px] border-[1.5px] border-ink bg-paper p-7 shadow-vexa"
            >
              <div className="vexa-display mb-2 text-[22px] text-vexa-red">
                {t(`${key}.ch`)}
              </div>
