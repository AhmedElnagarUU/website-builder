...
export function StickyNote({
  ...
  return (
    <div className={`relative -rotate-1 max-w-xs p-3 ${className}`}>
      <span
        aria-hidden
        className="mono-display absolute -top-1 -start-1 text-2xl leading-none text-mono-red"
      >
        ✱
      </span>
      <div className="rounded-[4px] border-[1.5px] border-ink bg-paper px-3.5 py-3 shadow-mono">
        {children}
      </div>
    </div>
  );
