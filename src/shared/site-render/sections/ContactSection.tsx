...
export function ContactSection({ content, businessInfo }: SectionRenderProps) {
  ...
  if (businessInfo.contactPhone) {
    rows.push({
      label: t("labels.tel"),
      node: (
        <a href={`tel:${businessInfo.contactPhone}`} className="underline underline-offset-2 hover:text-foreground">
  ...
  if (businessInfo.contactEmail) {
    rows.push({
      ...
      node: (
        <a href={`mailto:${businessInfo.contactEmail}`} className="underline underline-offset-2 hover:text-foreground">
      ...
      <div className="mx-auto grid max-w-5xl gap-12 px-4 @4xl:grid-cols-2">
        <div className="flex flex-col items-start justify-center gap-4">
          ...
          <F
            ...
            content={content}
            as="p"
            className="site-body max-w-xl leading-relaxed text-muted-foreground"
          />
        </div>
        <div
